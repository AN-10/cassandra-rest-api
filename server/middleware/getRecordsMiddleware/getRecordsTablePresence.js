...
module.exports = function() {
  return function tablePresence(req, res, next) {
    var con = mysql.createConnection({
      host: datasources.localDB.host,
      user: datasources.localDB.user,
      password: datasources.localDB.password,
      database: datasources.localDB.database
    });
    con.connect(function(err) {
      ...
      if (err) {
        ...
        var sql = "SELECT * from `table` where keyspaceName = (SELECT id from keyspace where keyspaceName ='" + req.params.keyspaceName + "' ) AND tableName = '" + req.body.tableName + "';";
