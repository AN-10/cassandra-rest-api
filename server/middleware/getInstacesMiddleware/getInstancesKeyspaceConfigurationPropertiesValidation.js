...
module.exports = function() {
  return function getInstancesKeyspaceConfigurationPropertiesValidation(req, res, next) {
    /*
     * To validate that JSON object (cassandra_node_configurations) has these properties (ip, port, keyspace_name)
     * in valid format
     */
    if (errorHandler.validateKeyspaceName(req.params.keyspaceName)) {// to validate that JSON object properties are valid
      next()
    } else {
